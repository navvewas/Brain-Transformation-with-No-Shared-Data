...
img_norm = np.load(dir+'/images/img_norm.npz')
...
class vgg_layer_loss():
    ...
    def loss_predict(self,y_true, y_pred):
        y_true_e = self.lossModel.predict(y_true)
        y_pred_e = self.lossModel.predict(y_pred)
        res = 0
        for i in range(len(y_true_e)):
            y_true_en = self.normalize(y_true_e[i])
            y_pred_en = self.normalize(y_pred_e[i])
...
def vgg_total_loss_predict(y_true, y_pred):
    return vgg_loss.loss_predict(y_true,y_pred)
...
def feature_loss_predict(y_true, y_pred ):
        return 10*vgg_total_loss_predict(y_true,y_pred)  + tf.dtypes.cast(mae(y_true,y_pred), tf.float32)
def combined_loss(y_true, y_pred):
    return feature_loss(y_true, y_pred)+  Tv_reg *total_variation_loss(y_pred)

def combined_loss_predict(y_true, y_pred):
    return feature_loss_predict(y_true, y_pred)+  Tv_reg * tf.dtypes.cast(total_variation_loss(y_pred), tf.float32) 

def combined_loss_fake(y_true, y_pred):
    return y_pred

def mae_vox_noSNR(FMRI_true, FMRI_pred):
    return K.mean(K.abs(FMRI_true-FMRI_pred),axis=-1)

def combined_voxel_loss_noSNR(FMRI_true, FMRI_pred, cosine_w=0.1):
    return mae_vox_noSNR(FMRI_true, FMRI_pred) +  cosine_w*cosine_proximity(FMRI_true, FMRI_pred)
