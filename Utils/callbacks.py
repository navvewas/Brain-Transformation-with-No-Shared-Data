...
class EpochProgressPrinter(keras.callbacks.Callback):
...
class save_model_callback(keras.callbacks.Callback):
    def __init__(self, model, dir = '',freq = 10):
        self.model = model
        self.freq = freq
        self.dir = dir
    def on_epoch_end(self, epoch, logs={}):
        if(epoch%self.freq==0):
            if not os.path.exists(self.dir):
                os.makedirs(self.dir)
            self.model.save_weights(self.dir + '.hdf5')

class log_image_collage_callback_encdec(keras.callbacks.Callback):
    def __init__(self, Y, X, model_enc,model_dec, dir = '',freq = 10):
        self.Y = Y
        self.X = X
        self.pred_model_enc = model_enc
        self.pred_model_dec = model_dec
        self.freq = freq
        self.dir = dir

    def on_epoch_end(self, epoch, logs={}):
        if(epoch%self.freq==0):
            X_pred = self.pred_model_dec.predict(self.pred_model_enc.predict(self.X))
            collage = image_collage([self.X,X_pred])
            if not os.path.exists(self.dir):
                os.makedirs(self.dir)
...
class corr_metric_callback(keras.callbacks.Callback):
    ...
    def __init__(self, train_data: object = None, test_data: object = None, generator: object = None, tensorboard_cb: object = None, num_voxels: object = 0,
                 pred_function: object = None,ROI_LVC = None, ROI_HVC = None,encoder_model =None,encoder_shared = None,name = '',vox_noise_ceil = None) -> object:
        super().__init__()
        ...
        self.name = name
        self.vox_noise_ceil = vox_noise_ceil
    def predict(self,Y):
        if(self.encoder_shared is not None):
            return self.encoder_model.predict(self.encoder_shared.predict(Y))
    ...
    def test_top_corr(self,data=None,genetator =None,mode=None, bacth_size=16, num_batches=30, top_k=100, ROI = None):
        ...
        if (self.vox_noise_ceil is not None):
            corr = corr[self.vox_noise_ceil != 0]
            corr = corr / self.vox_noise_ceil[self.vox_noise_ceil != 0]
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        print('train_' + self.name + ':'+str(train_corr_stat))
       # print('val:'+str(val_corr_stat))
        print('test_' + self.name + ':'+str(test_corr_stat))

        # self._data.append({
        #     'test_corr':test_corr,'val_corr': val_corr,'train_corr':train_corr
        # })

        self.write_log(['test_' + self.name + '/corr_median','train_' + self.name + '/corr_median'], [test_corr_stat[1],train_corr_stat[1]], epoch)
        self.write_log(['test_' + self.name + '/corr_mean','train_' + self.name + '/corr_mean'], [test_corr_stat[0],train_corr_stat[0]], epoch)
        self.write_log(['test_' + self.name + '/corr_75', 'train_' + self.name + '/corr_75'],[test_corr_stat[2],train_corr_stat[2]], epoch)
        self.write_log(['test_' + self.name + '/corr_90', 'train_' + self.name + '/corr_90'],[test_corr_stat[3],train_corr_stat[3]], epoch)
