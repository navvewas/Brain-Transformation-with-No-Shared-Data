...
if __name__ == '__main__':
    ...
    from Utils.params.training_params_transf import *
    ...
    subject_2 = eval(sys.argv[4]) 
    subject_student_2_data = eval(sys.argv[5])
    teacher_subject = eval(sys.argv[6]) # 0 no SS, else SS with which subject
    teacher_subject_data = eval(sys.argv[7]) # Subject data: 0- ImageNet dataset,  1 - NSD
    ref = eval(sys.argv[8]) # 0 - using SS (Self Supervised), 1- refrence, not using SS
    exp_array_s = [eval(sys.argv[9])] # shared example number
    exp_array_ns = [eval(sys.argv[10])] # non-shared example number
    exp_array_s_2 = [eval(sys.argv[11])] 
    exp_array_ns_2 = [eval(sys.argv[12])] 
    ...
    transf_weights_2_to_1  = prepare_all_params(encoder_transf_run,share_non_share_param,exp_array_s,exp_array_ns,exp_array_s_2,exp_array_ns_2,\
                                                exp_teacher_s_ns, subject_student_1_data,subject_student_2_data,teacher_subject_data,ref,subject_1,subject_2,teacher_subject)
    ...
    if(subject_student_2_data==1):
        ...
        dataset_2 = 'GOD'
    print('Training Transformation from Subject ' +str(subject_1) +',Dataset ' + str(dataset_1) + ', shared_examples=' + str(exp_s) + ', nonshared_examples='+str(exp_ns))
    print('Training Transformation to Subject ' +str(subject_2) +',Dataset ' + str(dataset_2) + ', shared_examples=' + str(exp_s_2) + ', nonshared_examples='+str(exp_ns_2))
    if (ref == 0):
        if(teacher_subject!=0):
            extra_tmp = extra_teacher.split('_exp')[0][1:]
        else:
            extra_tmp = ''
        print('Exploiting Data using Encoders ' + extra_tmp)
    ...
    test_images_1_s, test_FMRI_1_s, test_images_2_s, test_FMRI_2_s = load_data_and_encoders(encoder_transf_run,subject_student_1_data,subject_student_2_data,teacher_subject_data,\
                          subject_1,subject_2,teacher_subject,get_train_avg,exp_s,exp_s_2,exp_t_s,Voxel_to_choose,scale_1,scale_2,\
    ...
    test_model_and_print(transf_net_1_to_2,test_FMRI_1_s,test_FMRI_2_s, bacth_size=16, num_batches=30)
    #save models
