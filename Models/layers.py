...
class locally_connected_1d(Layer):
    ...
    def __init__(self, out=1,l1=0.1, l2=0, kernel_initializer = "glorot_normal"
,add_bias = True , constraint =None,single_dim = None,**kwargs):
    ...
    def build(self, input_shape):
        # Create a trainable weight variable for this layer.
        self.s = input_shape
        shape = [1]+list(input_shape[1:]) ### 1 = self.out = out
        if(self.single_dim is not None):
            shape[self.single_dim] =1
        regularizer = reg.l1_l2(l1=self.l1, l2=self.l2) if self.l1 and self.l2 else (reg.l1(self.l1) if self.l1 else (reg.l2(self.l2) if self.l2 else None))
...
class locally_connected_1d_new(Layer):

    def __init__(self, out=1,l1=0,l2 =0, kernel_initializer = "glorot_normal"
,add_bias = True , constraint =None,single_dim = None,**kwargs):
        self.l1 = l1
        self.l2 = l2
        self.out = out
        ...
        self.constraint = constraint
        self.single_dim = single_dim
        super(locally_connected_1d_new, self).__init__( **kwargs)
    ...
    def build(self, input_shape):
        ...
        if (self.out > 1 ):
            shape = shape + [self.out]  ### 1 = self.out = out
        if(self.single_dim is not None):
            shape[self.single_dim] =1

        self.kernel = self.add_weight(name='locally_connected_1d_kernel',
                                      shape=shape,
                                      regularizer= reg.l2(self.l1),
                                      initializer=self.kernel_initializer,
                                      constraint= self.constraint,
                                      trainable=True)
        if(self.add_bias):
            self.bias = self.add_weight(name='locally_connected_1d_bias', shape=([self.s[1],self.out]),initializer="glorot_normal",trainable=True)
        super(locally_connected_1d_new, self).build(input_shape)


    def call(self, x):
        output = tf.multiply(tf.expand_dims(x,-1), self.kernel)
        if (self.out  == 1):
            output = K.sum(output,axis=-1)
        else:
    ...
    def compute_output_shape(self, input_shape):
        if ( self.out > 1 ):
            return  tuple(list(input_shape[0:-1]) + [self.out])
        return input_shape[0:-1]